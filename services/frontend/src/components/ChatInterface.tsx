...
const ChatInterface: React.FC = () => {
  ...
  // Fetch Ollama/LM Studio models when provider changes
  useEffect(() => {
              ...
              <div>
                ...
                {(chatSettings.provider === 'ollama' || chatSettings.provider === 'lmstudio') ? (
                  <div className="space-y-2">
                          ...
                          {(chatSettings.provider === 'ollama' ? ollamaModels : lmstudioModels).map((model) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                setChatSettings(prev => ({ ...prev, model }));
                              }}
                              className="px-2 py-1 bg-gray-700 hover:bg-blue-600 rounded text-xs transition-colors"
                            >
                              {model}
                            </button>
                          ))}
                    ...
                    {!loadingModels && (chatSettings.provider === 'ollama' ? ollamaModels : lmstudioModels).length === 0 && (
                      <div className="text-sm text-yellow-400 bg-yellow-900/20 p-2 rounded border border-yellow-700/50">
                        ⚠️ No models detected. Make sure {chatSettings.provider === 'ollama' ? 'Ollama is running (ollama serve)' : 'LM Studio is running'}.
                      </div>
                    )}
                  </div>
                ) : (
